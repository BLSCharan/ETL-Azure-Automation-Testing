...
def get_azure_connection():
    ...
    logging.info(f"Azure Connection Established | ENV: {ENVIRONMENT}")
    return conn
...
def get_incremental_source(df_source, df_target):
    ...
    today = datetime.today().date()  #2026-09-10
...
def validate_dtypes(df_source, df_target):
    ...
    for col in df_source.columns:   #6 COLS
...
def validate_region_aggregations(df_target):
    ...
    #grouped -- 5 records joining tgt table on inner on region
...
def validate_aggregate_reconciliation(df_source, df_target):
    ...
    logging.info(f"Filtered Source Sales Sum round: {round(source_sum,2)}")
    logging.info(f"Target Sales Sum round: {round(target_sum,2)}")
...
def validate_hash_reconciliation(df_source, df_target):
    ...
    source_ids = set(filtered_source[PRIMARY_KEY_SOURCE])  #{2034,2035...}
    target_ids = set(df_target[PRIMARY_KEY_TARGET])  #{2034,2035...}
...
def validate_sampling_reconciliation(df_source, df_target, sample_size=100):
    ...
    sample_source = filtered_source.sample(min(sample_size, len(filtered_source)))#(100,3765)
