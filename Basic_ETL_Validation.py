...
def validate_counts():
    ...
    if source_count == target_count: #10=5
...
def validate_columns():
    source_cols = set(col.lower() for col in df_source.columns) #{id,name,city,age}
    print("source_cols",source_cols)
    target_cols = set(col.lower() for col in df_target.columns) #{id,name,city,age}
...
def validate_data_mismatch(primary_key):
    ...
    try:
        ...
        for col in df_source.columns:
            ...
            source_col = f"{col}_source" #NAME_SRC,CITY_SRC,AGE_SRC,ADDR_SRC
            target_col = f"{col}_target" #NAME_TGT,CITY_TGT,AGE_TGT,ADDR_TGT
