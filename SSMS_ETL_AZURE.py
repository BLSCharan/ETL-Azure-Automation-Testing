...
def extract_data():
    ...
    print(
        f"Extracted {len(df)} rows from SQL Server"
    )
...
def apply_all_transformations(
    ...
    region_dimension = create_region_dimension(
        df
    )

    df = join_fact_with_dimension(
        df,
        region_dimension
    )
...
def load_to_azure_sql(
    ...
    print(
        f"Data loaded into Azure SQL successfully "
        f"with LOAD_DT = {load_dt}"
    )
